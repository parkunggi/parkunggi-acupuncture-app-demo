 ...
 * APP_VERSION 20250922-NAV-LINK-HOTFIX6-REVERT-LINK+HIST-DROP
 *
 * 追加:
 *  - 履歴ドロップダウン UI
 *  - 直近最大60件表示/フィルタ/ジャンプ/全削除
 * 既存:
 *  - 4列CSVパーサ (経絡,経穴,部位,要穴)
 *  - 履歴 state: home / point / pattern / unknownPoint
 *  - URL 同期 (?point= / ?cat=&pattern=)
 *  - 履歴サイズ制限 HISTORY_MAX
 ******************************************************/
const APP_VERSION = '20250922-NAV-LINK-HOTFIX6-REVERT-LINK+HIST-DROP';
...
async function loadAcuCSV(){
  ...
  try{
    ...
    buildNameLookup();
    DATA_READY=true;
    statusEl.textContent=(ACUPOINTS.length===EXPECTED_TOTAL)
      ? `経穴CSV: ${ACUPOINTS.length}件 / 想定${EXPECTED_TOTAL}`
      : `経穴CSV: ${ACUPOINTS.length}件 (想定${EXPECTED_TOTAL})`;
  }catch(err){
