 ...
 * APP_VERSION 20251011-RANK-COLS-ONLY-SUGGEST-RANK-BADGES
 *
 * 追加 / 変更点 (前回 20251011-RANK-COLS-ONLY-SUGGEST-RANK からの差分):
 *  - サジェスト行右端にバッジ表示: ✦(要穴), M(筋肉<神経(筋枝)>) , N(神経(皮枝)), A(血管), Rank
 *  - 順序: ✦ → M → N → A → Rank
 *  - スターは赤系フレア “✦” に変更
 *  - 筋肉/神経/血管 検索対象は既存 extraTokens を改良しカテゴリ別 (muscleTokens / nerveTokens / vesselTokens) を保持
 *  - _hitMuscle/_hitNerve/_hitVessel フラグでマッチ種別をサジェストへ反映
 *  - 既存ソート (要穴→ランク S>A>B>C→名称) / UI / 他機能は変更なし
 ******************************************************/
const APP_VERSION = '20251011-RANK-COLS-ONLY-SUGGEST-RANK-BADGES';
...
const READINGS   = window.ACU_READINGS   || {}; // MUSCLE_MAP 廃止済み
...
function fallbackRenderPatternHistoryMenu(){ renderPatternHistoryMenu(); }
...
function fallbackRenderPointHistoryMenu(){ renderPointHistoryMenu(); }
...
function parseAcuCSV(raw){
  ...
  for(const rawLine of lines){
    ...
    if(cols[0]==='経絡' && cols[1] && cols[1].startsWith('経穴')){
      if(!headerMap){
        headerMap={};
        cols.forEach((h,i)=>{
          const key=h.replace(/[ \u3000]/g,'');
          if(key.startsWith('経絡')) headerMap.meridian=i;
          else if(key.startsWith('経穴')) headerMap.name=i;
          else if(key.startsWith('部位')) headerMap.region=i;
          else if(key.startsWith('要穴')) headerMap.important=i;
          else if(key.startsWith('筋肉<神経(筋枝)>')) headerMap.muscleBranch=i;
          else if(key.startsWith('神経(皮枝)')) headerMap.nerveCutaneous=i;
          else if(/^血管?$/.test(key) || key==='血菅') headerMap.bloodVessel=i;
          else if(key.startsWith('ランク')) headerMap.rank=i;
        });
      }
      continue;
    }
...
/* ------------ 組織/神経/血管トークン生成 (カテゴリ別) ------------ */
function buildExtraTokensForPoint(p){
  const muscleTokens=[], nerveTokens=[], vesselTokens=[], all=[];
  function add(targetArr, t){
    const tok=trimOuter(removeAllUnicodeSpaces(t||''));
    if(!tok) return;
    if(tok.length===1 && /[ぁ-んゃゅょやのと及又]/.test(tok)) return;
    if(!targetArr.includes(tok)) targetArr.push(tok);
    if(!all.includes(tok)) all.push(tok);
  }
  function splitGeneral(str){
    return (str||'')
      .replace(/[＋+·・]/g,'・')
      .replace(/[／/]/g,'/')
      .split(/[/・]/);
  }
  function handleMuscleBranch(str){
    if(!str) return;
    str.split(/[／/]/).forEach(seg=>{
      ...
      if(m){
        ...
        if(outside) splitGeneral(outside).forEach(s=>add(muscleTokens,s));
        const inside=m[2].trim();
        if(inside){
          inside.split(/[\(\)（）]/).filter(Boolean).forEach(part=>{
            splitGeneral(part).forEach(s=>add(muscleTokens,s));
          });
        }
      }else{
        splitGeneral(seg).forEach(s=>add(muscleTokens,s));
      }
    });
  }
  function handleSimple(str,target){
    if(!str) return;
    str.split(/[／/]/).forEach(seg=>{
      seg=seg.trim(); if(!seg) return;
      splitGeneral(seg).forEach(s=>add(target,s));
    });
  }
  handleMuscleBranch(p.muscleBranch);
  handleSimple(p.nerveCutaneous, nerveTokens);
  handleSimple(p.bloodVessel, vesselTokens);

  p.muscleTokens = muscleTokens;
  p.nerveTokens  = nerveTokens;
  p.vesselTokens = vesselTokens;
  p.extraTokens  = all;
}
...
function isTreatmentHeaderCell(cell){
  return /治療方針/.test(removeAllUnicodeSpacesSimple(cell||''));
}
...
function showPointDetail(p, suppressHistory=false){
  ...
  if(resultMuscleEl) resultMuscleEl.textContent = p.muscleBranch || '-';
  if(resultNerveCutaneousEl) resultNerveCutaneousEl.textContent = p.nerveCutaneous || '-';
  if(resultBloodVesselEl) resultBloodVesselEl.textContent = p.bloodVessel || '-';
...
function showUnknownPoint(name, suppressHistory=false){
  ...
  if(resultMuscleEl) resultMuscleEl.textContent='-';
  if(resultNerveCutaneousEl) resultNerveCutaneousEl.textContent='-';
  if(resultBloodVesselEl) resultBloodVesselEl.textContent='-';
  relatedSymptomsEl.innerHTML='<li>-</li>';
  inlineAcupointResult.classList.remove('hidden');
  hideMeridianImage();
  if(!suppressHistory && !IS_APPLYING_HISTORY){
...
function filterPoints(qInput){
  ...
  if(/^[\u3041-\u3096]+$/.test(q)){
    ...
    return sortSuggestionList(list.map(p=>({...p,_matchType:'name'})));
  }
  const seen=new Set();
  const out=[];
  // 経穴名
  for(const p of ACUPOINTS){
    if(p.name.includes(q)){
      out.push({...p,_matchType:'name',
        _hitMuscle:false,_hitNerve:false,_hitVessel:false});
      seen.add(p.name);
    }
  }
  // 組織マッチ (筋肉/神経/血管)
  for(const p of ACUPOINTS){
    ...
    const hitMuscle = p.muscleTokens?.some(t=>t.includes(q)) || false;
    const hitNerve  = p.nerveTokens?.some(t=>t.includes(q))  || false;
    const hitVessel = p.vesselTokens?.some(t=>t.includes(q)) || false;
    if(hitMuscle||hitNerve||hitVessel){
      out.push({...p,_matchType:'tissue',
        _hitMuscle:hitMuscle,_hitNerve:hitNerve,_hitVessel:hitVessel});
      seen.add(p.name);
    }
  }
  return sortSuggestionList(out);
}
function sortSuggestionList(list){
  return list.slice().sort((a,b)=>{
    const impA = a.important?1:0;
    const impB = b.important?1:0;
    if(impA!==impB) return impB-impA;
    const ra = (RANK_ORDER[a.rank]!==undefined)?RANK_ORDER[a.rank]:9;
    const rb = (RANK_ORDER[b.rank]!==undefined)?RANK_ORDER[b.rank]:9;
...
function renderSuggestions(list){
  ...
  list.slice(0,120).forEach((p,i)=>{
    ...
    const badges=[];
    if(important){
      badges.push('<span class="badge-mini badge-star">✦</span>');
    }
    if(p._hitMuscle){
      badges.push('<span class="badge-mini badge-m">M</span>');
    }
    if(p._hitNerve){
      badges.push('<span class="badge-mini badge-n">N</span>');
    }
    if(p._hitVessel){
      badges.push('<span class="badge-mini badge-a">A</span>');
    }
    if(p.rank){
      badges.push(`<span class="badge-mini badge-rank">${escapeHTML(p.rank)}</span>`);
    }
    ...
    li.innerHTML=`
      ...
      <span class="sug-rank">${badges.join(' ')}</span>`;
...
async function loadAcuCSV(){
  ...
  try{
    ...
    ACUPOINTS.forEach(buildExtraTokensForPoint);
    buildNameLookup();
    DATA_READY=true;
